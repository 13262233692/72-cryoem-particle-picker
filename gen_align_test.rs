...
fn make_gaussian_image(nx: i32, ny: i32, cx: f32, cy: f32, sigma: f32, amplitude: f32) -> Vec<f32> {
    ...
    for y in 0..ny {
        for x in 0..nx {
            ...
            let noise = (x as u64 * 7919 + y as u64 * 6271 + 9999) % 100000;
            let noise_f = noise as f32 / 50000.0 - 1.0;
            data.push(100.0 + g + noise_f * 5.0);
        }
    }
    data
}
...
fn main() {
    ...
    write_mrc("ref_align.mrc", nx, ny, &ref_img);
    println!("Created ref_align.mrc: particle at ({}, {})", ref_cx, ref_cy);
    ...
    write_mrc("tgt_align.mrc", nx, ny, &tgt_img);
    println!("Created tgt_align.mrc: particle at ({}, {})", tgt_cx, tgt_cy);
